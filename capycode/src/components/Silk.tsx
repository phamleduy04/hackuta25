...
interface UniformValue<T = number | any> {
  value: T;
}
...
const SilkPlane = React.forwardRef<any, SilkPlaneProps>(function SilkPlane(
  { uniforms },
  ref
) {
  ...
  useLayoutEffect(() => {
    const mesh = ref as React.MutableRefObject<any | null>;
    if (mesh.current) {
...
void main() {
  ...
  float pattern = 0.6 +
                  0.4 * sin(1.5 * (tex.x + tex.y +
                                   cos(1.0 * tex.x + 2.0 * tex.y) +
                                   0.02 * tOffset) +
